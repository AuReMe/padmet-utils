...
def run_sbml_to_sbml(multiprocess_data):
    ...
    Args:
        multiprocess_data (dictionary): pathname to species sbml file, pathname to output sbml file, new sbml version level
    ...
    padmet_to_sbml(padmet, multiprocess_data['sbml_output_file'], sbml_lvl=multiprocess_data['new_sbml_version'], verbose=False)
...
def from_sbml_to_sbml(input_sbml, output_sbml, new_sbml_version, cpu):
    """Turn sbml to sbml.

    Args:
        input_sbml (string): pathname to species sbml file/folder
        output_sbml (string): pathname to output sbml file/folder
        new_sbml_version (int): new sbml version level
    ...
    """
    try:
        new_sbml_version = int(new_sbml_version)
    except:
        print('SBML level must be an int.')

    if new_sbml_version not in [2, 3]:
    ...
    if os.path.isdir(input_sbml):
        ...
        for sbml_file in os.listdir(input_sbml):
            ...
            multiprocess_data['new_sbml_version'] = new_sbml_version
...
def call_sbml_to_sbml():
    ...
    new_sbml_version = args["--new_sbml_lvl"]
    ...
    from_sbml_to_sbml(input_sbml, output_sbml, new_sbml_version, cpu)
