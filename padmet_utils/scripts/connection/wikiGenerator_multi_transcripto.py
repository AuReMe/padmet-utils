...
::
    ...
    usage:
        wikiGenerator.py --padmetSpec=FILE --output=DIR --wiki_id=STR [--padmetRef=FILE] [--log_file=FILE] [-v]
...
def main(): 
    global padmetSpec, padmetRef, wiki_folder, full_sources_dict, all_categories, all_tools, all_sources, total_pwy_id, total_cpd_id, all_rxns, all_genes, all_pwys, all_cpds, all_diffCdt, all_orgs, verbose, ext_link
    ...
    all_diffCdt = [node for node in padmetSpec.dicOfNode.values() if node.type == "diffCondition"]
    ...
    for diffCdt_node in all_diffCdt:
        create_biological_page("diffCondition", diffCdt_node, wiki_folder+"misc/")        
...
def create_navigation_page(output_folder):
    ...
    if all_orgs:
        ...
        dataInArray = ["{{#ask: [[Category:Organism]]","| ?taxon","| ?locus",
