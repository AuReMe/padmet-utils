...
def main():
    ...
    for sbml_rxn in listOfReactions:
        ...
        #first check in inter dict mapping
        match_id = intern_mapping(rxn_id, db_out, "reaction")
        
        #check if in mnx_rxn_dict
        if match_id:
            mapped_rxn[rxn_id] = match_id
        else:
            for mapp_dict in mnx_rxn_dict.values():
                #print(mapp_dict)
                all_rxn_id = []
                [all_rxn_id.extend(i) for i in mapp_dict.values()]
                if uncoded in all_rxn_id:
                    matchs_rxns = mapp_dict[db_out]
                    if len(matchs_rxns) > 1: 
                        rxn_with_more_one_mapping += 1
                        if verbose:
                            print("more than one mapping for:%s" %rxn_id)
                            print("\t%s" %matchs_rxns)
                    else:
                        mapped_rxn[rxn_id] = matchs_rxns[0]
                    break
    for sbml_rxn in [i for i in listOfReactions if i.id not in mapped_rxn.keys()]:
        all_cpds = [r.getSpecies() for r in sbml_rxn.getListOfReactants()] + [r.getSpecies() for r in sbml_rxn.getListOfProducts()]
        for cpd_id in all_cpds:
            uncoded_cpd_id = convert_from_coded_id(cpd_id)[0]

    print("Mapped reactions: %s" %(len(mapped_rxn.keys())))            
    exit()
...
def intern_mapping(id_to_map, db_out, _type):
    ...
    intern_cpd_dict = {
    "UNIQ_ID_1":{"METACYC":["WATER"],"KEGG":["C00001"],"BIGG":["h2o"]},
    ...
    elif id_to_map in ["nadph","nadp","nadh","nad","hco3","ACP","hso3",
    "gly","ppi","amp","cdp","cmp","udp","ump","gdp","adp"]:
        return id_to_map.upper()
