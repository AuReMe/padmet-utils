...
def main(): 
    global padmetSpec, padmetRef, wiki_folder, full_sources_dict, all_categories, all_tools, all_sources, total_pwy_id, total_cpd_id, all_rxns, all_genes, all_pwys, all_cpds, verbose, ext_link, all_orgs, all_diffExp
    ...
    """
    a = PadmetSpec()
    a.setInfo(padmetSpec)
    a.setPolicy(padmetSpec)
    a.copyNode(padmetSpec,'H3FWH5FW_VS_H10FW')
    padmetSpec = a
    """
    ...
    all_orgs = [node for node in padmetSpec.dicOfNode.values() if node.type == "organism"]
    for org_node in all_orgs:
        create_biological_page("Organism", org_node, wiki_folder+"organisms/")        
...
def createDirectory():
    ...
    dirNames = ["genes","reactions","metabolites","pathways","navigation","files",'organisms']
...
def create_navigation_page(output_folder):
    ...
    fileName = output_folder+"Category:Organism"
    if verbose: print("Category: %s" %category)
    dataInArray = ["{{#ask: [[Category:Organism]]",
                      "| ?Nb reactions associated","| ?Nb genes associated", "| ?Nb pathways associated","}}"]
    with open(fileName,'w') as f:
    ...
    dataInArray = ["{{#ask: [[Category:Reaction]]","| ?common name","| ?ec number",
                      "| ?organism associated","| ?gene associated","| ?in pathway","}}"]
    with open(fileName,'w') as f:
        for line in dataInArray:
            f.write(line+"\n")
    ...
    if verbose: print("Category: %s" %category)
    dataInArray = ["{{#ask: [[Category:Gene]]", "| ?reaction associated", "| ?pathway associated","| ?organism associated" ,"}}"]
    with open(fileName,'w') as f:
        for line in dataInArray:
            f.write(line+"\n")
    ...
    if verbose: print("Category: %s" %category)
    dataInArray = ["{{#ask: [[Category:Pathway]]","| ?common name","| ?all reaction found","| ?total reaction","| ?completion rate",]
    dataInArray += ["| ?reaction in "+org.id for org in all_orgs] + ["}}"]
    with open(fileName,'w') as f:
        for line in dataInArray:
            f.write(line+"\n")
...
def create_biological_page(category, page_node, output_folder):
    ...
    if category == "Reaction":
        ...
        org_associated = [rlt.id_out for rlt in padmetSpec.dicOfRelationIn[page_node.id] if rlt.type == "is_in_organism"]
        for org_id in org_associated:
            dataInArray.append('* [[%s]]' %org_id)
        add_property(properties, "organism associated", org_associated)
        ...
        if linked_rlt:
            ...
            for rlt in linked_rlt:
                ...
                organism_assign_list = rlt.misc["ORGANISM:ASSIGNMENT"]
                for organism_assign in organism_assign_list:
                    #ValueError: can't split and get 2 value == no known assignment
                    try:
                        organism, assignment = organism_assign.split(":")
                    except ValueError:
                        organism = organism_assign
                        assignment = None
                    dataInArray.append("** Orgnaism: [[%s]]" %organism)
                    if assignment:
        ...
        dataInArray.append('== Transcriptomic data  ==')
        dataInArray.append('* DESeq analysis:')
        is_de_in_rlts = [rlt for rlt in padmetSpec.dicOfRelationIn[page_node.id] if rlt.type == "is_de_in"]
        if is_de_in_rlts:
            for rlt in is_de_in_rlts:
                dataInArray.append("** DiffExp [[%s]]:" %rlt.id_out)
                add_property(properties, "LogDiffExp %s"%rlt.id_out, [rlt.misc['log2FoldChange']])
                for k,v in rlt.misc.items():
                    dataInArray.append("*** %s" %k)
                    dataInArray.append("**** %s" %[v])
        else:
            dataInArray.append("** NA")
        dataInArray.append('* Reads count:')
        has_reads_in_rlts = [rlt for rlt in padmetSpec.dicOfRelationIn[page_node.id] if rlt.type == "has_reads_in"]
        if has_reads_in_rlts:
            for rlt in has_reads_in_rlts:
                dataInArray.append("** Condition [[%s]]:" %rlt.id_out)
                dataInArray.append("*** %s" %rlt.misc['NB-READS'][0])
        else:
            dataInArray.append("** NA")
        ...
        add_property(properties, "all reaction found", [len(reactionsFound)])
        add_property(properties, "total reaction", [len(reactionsTotal)])
        add_property(properties, "completion rate", [pwy_ratio])
        #TODO: create dict to add final property: Reaction in org_X
        dict_rxn_org_genes = {}
        for rxn_id in reactionsFound: dict_rxn_org_genes[rxn_id] = {}
        for rxn_id in reactionsFound:
            all_org_in = [rlt.id_out for rlt in padmetSpec.dicOfRelationIn[rxn_id] if rlt.type == "is_in_organism"]
            for org in all_org_in: dict_rxn_org_genes[rxn_id][org] = set() 
            gene_assoc = [rlt for rlt in padmetSpec.dicOfRelationIn[rxn_id] if rlt.type == "is_linked_to"]
            for gene_rlt in gene_assoc:
                gene_id = gene_rlt.id_out
                try:
                    org,assign = gene_rlt.misc['ORGANISM:ASSIGNMENT'][0].split(':')
                except ValueError:
                    org = gene_rlt.misc['ORGANISM:ASSIGNMENT'][0]
                    assign = 'NA'
                dict_rxn_org_genes[rxn_id][org].add(gene_id)
        for rxn_id, rxn_dict in dict_rxn_org_genes.items():
            dataInArray.append("* [["+rxn_id+"]]")
            nb_genes = sum([len(set_genes) for set_genes in rxn_dict.values()])
            dataInArray.append("** %s associated gene(s):" %(nb_genes))
            for org, set_genes in rxn_dict.items():
                add_property(properties, "reaction in %s"%org, [rxn_id])
                if set_genes:
                    genes_str = " and ".join(['[['+gene_id+']]' for gene_id in set_genes])
                    dataInArray.append("*** %s from [[%s]]" %(genes_str, org))
                else:
                    dataInArray.append("** From [[%s]] (No genes associated)" %org)
        ...
        rxn_in = [rlt.id_in for rlt in padmetSpec.dicOfRelationOut[page_node.id] if rlt.type == "is_in_organism" and padmetSpec.dicOfNode[rlt.id_in].type == "reaction"]
        pwys_in = set()
        for rxn_id in rxn_in:
            ...
            pwys_in.update(pwys)
        nb_genes = len([rlt.id_in for rlt in padmetSpec.dicOfRelationOut[page_node.id] if rlt.type == "is_in_organism" and padmetSpec.dicOfNode[rlt.id_in].type == "gene"])
        add_property(properties, "Nb reactions associated", [len(rxn_in)])
        add_property(properties, "Nb genes associated", [nb_genes])
        add_property(properties, "Nb pathways associated", [len(pwys_in)])
            
    elif category == "diffExpression":
        rxn_up = [rlt.id_in for rlt in padmetSpec.dicOfRelationOut[page_node.id] if rlt.type == "is_de_in" and padmetSpec.dicOfNode[rlt.id_in].type == "reaction" and float(rlt.misc["log2FoldChange"][0]) > 0]
        rxn_down = [rlt.id_in for rlt in padmetSpec.dicOfRelationOut[page_node.id] if rlt.type == "is_de_in" and padmetSpec.dicOfNode[rlt.id_in].type == "reaction" and float(rlt.misc["log2FoldChange"][0]) < 0]
        pwys_up = set()
        pwys_down = set()
        dataInArray.append("== Reaction(s) up-regulated ==")
        for rxn_id in rxn_up:
            dataInArray.append("* [[%s]]" %rxn_id)
            pwys = set([rlt.id_out for rlt in padmetSpec.dicOfRelationIn[rxn_id] if rlt.type == "is_in_pathway" and rlt.id_out in total_pwy_id])
            pwys_up.update(pwys)
        dataInArray.append("== Pathway(s) up-regulated ==")
        for pwy_id in pwys_up:
            dataInArray.append("* [[%s]]" %pwy_id)
        dataInArray.append("== Reaction(s) down-regulated ==")
        for rxn_id in rxn_down:
            dataInArray.append("* [[%s]]" %rxn_id)
            pwys = set([rlt.id_out for rlt in padmetSpec.dicOfRelationIn[rxn_id] if rlt.type == "is_in_pathway" and rlt.id_out in total_pwy_id])
            pwys_down.update(pwys)
        dataInArray.append("== Pathway(s) down-regulated ==")
        for pwy_id in pwys_down:
            dataInArray.append("* [[%s]]" %pwy_id)

        nb_genes = len([rlt.id_in for rlt in padmetSpec.dicOfRelationOut[page_node.id] if rlt.type == "is_in_organism" and padmetSpec.dicOfNode[rlt.id_in].type == "gene"])
        add_property(properties, "reaction up-regulated", rxn_up)
        add_property(properties, "reaction down-regulated", rxn_down)
